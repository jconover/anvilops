...
data "aws_iam_policy_document" "puppet_permissions" {
  ...
  statement {
    sid    = "SecretsManagerWritePuppetToken"
    effect = "Allow"
    actions = [
      "secretsmanager:PutSecretValue",
    ]
    resources = [
      "arn:aws:secretsmanager:*:${data.aws_caller_identity.current.account_id}:secret:anvilops/puppet-*",
    ]
  }
}
...
###############################################################################
# 10. GitHub Actions OIDC Provider + Deploy Role
###############################################################################
...
resource "aws_iam_openid_connect_provider" "github_actions" {
  ...
  tags = merge(local.merged_tags, { Component = "github-actions-oidc" })
}
...
data "aws_iam_policy_document" "github_actions_trust" {
  ...
  statement {
    ...
    condition {
      ...
      values   = [for repo in var.github_actions_repos : "repo:${repo}:*"]
    }
  }
}
...
resource "aws_iam_role" "github_actions_deploy" {
  ...
  tags               = merge(local.merged_tags, { Component = "github-actions-deploy" })
}
...
data "aws_iam_policy_document" "github_actions_deploy_permissions" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:eks:*:${data.aws_caller_identity.current.account_id}:cluster/${var.project_name}-*",
    ]
  }
}
