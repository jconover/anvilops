...
locals {
  user_data_vars = {
    ...
    puppet_api_token_secret_arn = aws_secretsmanager_secret.puppet_api_token.arn
  }
}
...
resource "aws_kms_key" "secrets" {
  description             = "CMK for AnvilOps Puppet Secrets Manager secrets"
...
resource "aws_kms_alias" "secrets" {
  name          = "alias/${var.project_name}-${var.environment}-puppet-secrets"
...
# -----------------------------------------------------------------------------
# Secrets Manager — PE API Token (written by user_data after installation)
# -----------------------------------------------------------------------------

resource "aws_secretsmanager_secret" "puppet_api_token" {
  name                    = "anvilops/puppet"
  description             = "Puppet Enterprise RBAC API token for ${var.environment} (auto-generated by user_data)"
  recovery_window_in_days = 7
  kms_key_id              = aws_kms_key.secrets.arn
